...
Select provinces and press Done, then use **Local state report → Inspect** to view the current local tool/remote/Humanoid state. The diagnostic loader locks that instance to zero AutoPainter requests, including after Q/Safe/Fast/Toggle Paint. Inspect again after equipping or respawning to refresh the snapshot. Use **Prev / Next** to copy successive report pages. The separate **PAINTBUCKET DESCENDANTS** section uses one native `PaintBucket:GetDescendants()` snapshot, lists every returned descendant in full-path order, and ends with listed/expected counts plus `FreshRemoteInInventory`. This version refuses older diagnostic panels; close the old panel and reload to get UI version 3 (inventory format 2, source format 1). No probing calls or test paint are performed. Existing already-sent calls and independently running normal tool scripts cannot be canceled by this mode.

**PaintBucket LocalScript source → Inspect source** is a separate, explicit read-only action available only while diagnostics are LOCKED ON. It checks PaintBucket tools under your current Character and Backpack, then calls the existing environment-provided `decompile` function only for their LocalScript descendants (including LocalScript, RemoteScript and VipTextures when present). The **PAINTBUCKET LOCALSCRIPT SOURCE** report includes full paths, complete returned source, explicit unavailable/failure status and a line-numbered text index for PaintPart/remotes, Peace, colors, equip/activation, mode/state, timing and client responses. Use Prev / Next for long reports. `GetLocalScriptReport()` returns the entire report string. Existing Inspect remains metadata-only and never decompiles.

The optional decompiler is supplied by your existing client environment; AutoPainter cannot certify that implementation or its output. There is no fallback through script Source, closures, modules, hooks or game calls. Returned text is displayed locally without being executed, uploaded or copied automatically. Only one inspection runs at a time; repeated clicks create no waiting jobs. If the external decompiler never returns, that inspection stays busy until it returns or the client session ends.
