...
source = (root / "AutoPainterDiagnostics.luau").read_text()
loader = (root / "LoaderPublic.luau").read_text()
diagnostic_loader = (root / "LoaderDiagnostic.luau").read_text()
suite = (root / "tests" / "scheduler.spec.luau").read_text().split("for _, deferred", 1)[0]
suite += "\n" + (root / "tests" / "selection.spec.luau").read_text().split("for _, deferred", 1)[0]
...
suite += '\nprint(string.format("%d diagnostic tests passed (mock engine; no live scan claim)", passed))\n'
